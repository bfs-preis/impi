...
export function read(xmlstring, callback) {
    ...
    saxparser.onopentag = function (node: { attributes: any; name: string | number; }) {
        // opened a tag.  node has "name" and "attributes"

        // create a new object and fill it with a function that returns the attributes:
        const newobject = {
            attributes: function (_name) {
        ...
        if (object[node.name]) {
            ...
            if (!object[node.name].array) {
                ...
                const firstobject = object[node.name];
                object[node.name] = {};
    ...
    saxparser.onclosetag = function (_node: any) {
